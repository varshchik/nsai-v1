...
TESTS = [
    ...
    # Сиблинги через conj/appos/nmod/amod-NOUN присоединяются к args.
    # Без запятых natasha выбирает «мясо» как obj, а «рыбу» цепляет
    # к нему как amod-NOUN (аномалия дерева). Ролевой порядок:
    # nsubj → obj → сиблинги в порядке возврата natasha.
    # С запятыми («кот ест рыбу, мясо, молоко») natasha дала бы
    # obj=рыба, conj=мясо, conj=молоко → [кот, рыба, мясо, молоко].
    ("ENUM: перечисление",
     "кот ест рыбу мясо молоко",
     [("есть", ["кот", "мясо", "рыба", "молоко"])], []),
]
